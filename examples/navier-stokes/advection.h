...
// *****************************************************************************
// This QFunction implements the Jacobian of the Advection equations
//
// Discrete Jacobian: dF/dq^n = sigma * dF/dqdot|q^n + dF/dq|q^n ("sigma * dF/dqdot|q^n" will be added later)
// *****************************************************************************

CEED_QFUNCTION(JacAdvection)(void *ctx, CeedInt Q,
                          const CeedScalar *const *in, CeedScalar *const *out) {
  // Inputs
  const CeedScalar (*q)[Q] = (CeedScalar(*)[Q])in[0],
                   (*deltaq)[5][Q] = (CeedScalar(*)[5][Q])in[1],
                   (*qdata)[Q] = (CeedScalar(*)[Q])in[2],
                   (*x)[Q] = (CeedScalar(*)[Q])in[3];
  // Outputs
  CeedScalar (*v)[Q] = (CeedScalar(*)[Q])out[0],
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    // -- Grad in                                
    /*
    const CeedScalar deltarho[3]    =   {deltaq[0][0][i],
                                         deltaq[1][0][i],
                                         deltaq[2][0][i]
                                        };
    const CeedScalar deltau[3][3]   = {{(deltaq[0][1][i] - drho[0]*u[0]) / rho,
                                        (deltaq[1][1][i] - drho[1]*u[0]) / rho,
                                        (deltaq[2][1][i] - drho[2]*u[0]) / rho},
                                       {(deltaq[0][2][i] - drho[0]*u[1]) / rho,
                                        (deltaq[1][2][i] - drho[1]*u[1]) / rho,
                                        (deltaq[2][2][i] - drho[2]*u[1]) / rho},
                                       {(deltaq[0][3][i] - drho[0]*u[2]) / rho,
                                        (deltaq[1][3][i] - drho[1]*u[2]) / rho,
                                        (deltaq[2][3][i] - drho[2]*u[2]) / rho}
                                      };
                                      */
    const CeedScalar deltaE[3]      =   {deltaq[0][4][i],
                                         deltaq[1][4][i],
                                         deltaq[2][4][i]
                                        };
    // -- Interp-to-Interp qdata
 ...
 /*
    ...
    case 1: {                                                */
      ...
      for (int j=0; j<3; j++)
        dv[j][4][i] = wJ * deltaE[j] * (u[0]*dXdx[j][0] + u[1]*dXdx[j][1] + u[2]*dXdx[j][2]);
         v[4][i]    = 0;                                            /*
    } break;
