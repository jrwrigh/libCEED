...
#ifndef densitycurrent_stab_h
#define densitycurrent_stab_h
...
//
// *****************************************************************************
static int ICsDCStab(void *ctx, CeedInt Q,
...
//
// *****************************************************************************
static int DCStab(void *ctx, CeedInt Q,
              const CeedScalar *const *in, CeedScalar *const *out) {
  // Inputs
  ...
  for (CeedInt i=0; i<Q; i++) {
    ...
    const CeedScalar du[3][3] = {{(dU[0][0] - drho[0]*u[0]) / rho,
                                  (dU[0][1] - drho[1]*u[0]) / rho,
                                  (dU[0][2] - drho[2]*u[0]) / rho},
                                 {(dU[1][0] - drho[0]*u[1]) / rho,
                                  (dU[1][1] - drho[1]*u[1]) / rho,
                                  (dU[1][2] - drho[2]*u[1]) / rho},
                                 {(dU[2][0] - drho[0]*u[2]) / rho,
                                  (dU[2][1] - drho[1]*u[2]) / rho,
                                  (dU[2][2] - drho[2]*u[2]) / rho}
                                };                            
    // -- Interp-to-Interp qdata
    const CeedScalar wJ       =    qdata[i+ 0*Q];
    // -- Interp-to-Grad qdata --- Symmetric 3x3 matrix
    ...
    const CeedScalar gradT[3] = {( dE[0]/rho - E*drho[0]/(rho*rho) -
                                 ...
                                 ( u[0]*du[0][2] + u[1]*du[1][2] + u[2]*du[2][2])) / cv
                                };
    
    // -- Fuvisc ---- Symmetric 3x3 matrix
